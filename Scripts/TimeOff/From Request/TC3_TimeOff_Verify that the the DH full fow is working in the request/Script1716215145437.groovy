...
def date = new Date().plus(2).format('dd/MM/yyyy')
...
WebUI.setText(findTestObject('Page_Login/input_Email'), findTestData('Employees under Director').getValue(1, randomNumber).trim())
...
String TimeOffDescription = 'SH Time off in req ' + datetime
...
WebUI.verifyElementText(findTestObject('TimeOff/Responsible Approver'), findTestData('Employees under Director').getValue(
        2, randomNumber))
...
WebUI.setText(findTestObject('Object Repository/Page_Login/input_Email'), findTestData('Employees under Director').getValue(
        3, randomNumber).trim())
...
WebUI.click(findTestObject('TimeOff/Approve by HR button'))

WebUI.delay(2)

'HR Approve request'
WebUI.verifyElementAttributeValue(findTestObject('TimeOff/Stage Approve'), 'title', 'Current state', 0)
